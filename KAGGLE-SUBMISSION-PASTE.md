# Kaggle Submission — Paste Sheet

Open the in-progress Writeup draft at:
https://www.kaggle.com/competitions/gemma-4-good-hackathon/writeups

**Do NOT click "New Writeup"** — open the existing reserved draft. Then fill the form field by field with the values below.

---

## Field 1 — Title (80 char max)
...
## Field 2 — Writeup URL slug (auto-generated; confirm or set)

```
healthpulse-edge
```

Final URL becomes: `kaggle.com/competitions/gemma-4-good-hackathon/writeups/healthpulse-edge`

---

## Field 3 — Subtitle (140 char max)
...
## Field 4 — Submission Tracks (dropdown)

**Primary:** `Digital Equity & Inclusivity`

**Special Technology (if a second dropdown appears):** `Ollama`

If only one dropdown shows, the Ollama claim is already declared in the WRITEUP body table.

---

## Field 5 — Card and Thumbnail Image (required, 560×280)

Upload: `assets/cover-thumb.png`

(NOT `cover.png` — that's the larger 1200×630 hero used in Media Gallery.)

---

## Field 6 — Project Description (Markdown body — paste verbatim)

Word count: **1,444 / 1,500** (verify against Kaggle's built-in counter — must show ≤ 1,500 before submit).

Paste everything between the `---BEGIN-BODY---` and `---END-BODY---` markers below:
...
## Field 7 — Media Gallery (videos + images)

Add in this order:

| Order | Type | Source | Why |
|---|---|---|---|
| 1 | YouTube URL | `https://youtu.be/5hkNbITM5d4` | The video is the most important artifact per Kaggle rules |
| 2 | Image | `assets/cover.png` (1200×630) | Canonical hero |
| 3 | Image | `assets/screenshot-edge.png` | /edge live demo with WebGPU + ledger visible |
| 4 | Image | `assets/screenshot-sovereignty-block.png` | Money shot — Sovereignty REQUIRES SIGNATURE card |
| 5 | Image | `assets/screenshot-onprem-home.png` | On-prem app home with /edge callout |

---

## Field 8 — Project Links (Attachments)
...
(YouTube URL also goes in Media Gallery above; it doesn't need a duplicate entry here.)

---

## Field 9 — Files (optional, max 100 MB)

Skip unless Kaggle complains about a missing required attachment.

---

## Pre-submit final checks (60-second drill)

Before clicking **Submit** in the top-right of the Kaggle Writeup form:

1. **Save the draft first** (top-right area or Cmd+S). Re-open to verify the body renders correctly.
2. **Watch Kaggle's built-in word counter** — must show ≤ 1,500 (the paste above is 1,444).
3. **Open the YouTube link in a new tab** — confirms it loads publicly without sign-in.
4. **Open the live demo link** — confirms `gemma-health.vercel.app/edge` returns the page.
5. **Open the GitHub repo URL in an incognito window** — confirms the public flip stuck (https://github.com/sgharlow/gemma-health, just made public).
6. **Click Submit**.
7. **Open the submitted Writeup in an incognito window** — confirm video + demo + repo all reachable without login.

---

## After submit

- Post the Kaggle Writeup URL to LinkedIn (template in `docs/LINKEDIN-POST.md`).
